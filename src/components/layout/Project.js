...
export default function Project({ project }) {
  const { title, problem, solution, stack, image, siteLink, repositories } =
    project;

  return (
    <div className="flex flex-col text-[#172432]">
      <div className="w-full mb-9">
        <h3 className="mb-3 font-semibold text-4xl md:text-3xl text-[#172432]">
          {title}
        </h3>
      ...
      <div className="flex flex-col lg:flex-row md:gap-16">
        ...
        <div className="lg:w-1/2">
          <p>
            <span className="font-bold">Problema identificado:</span> {problem}
          </p>
          <br />
          <p>
            <span className="font-bold">Solución:</span> {solution}
          </p>
          <br />
              ...
              {repositories.map((repo, index) => {
                ...
                return repoLink ? (
                  <li className="mb-3" key={index}>
                    <a
                      className="text-[#172432] underline hover:text-[#E25A27]"
                      href={repoLink}
                      target="_blank"
